...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, QTabWidget,
                             ...
                             QToolButton, QDialogButtonBox, QScrollArea, QRadioButton, QMessageBox, QProgressBar)
...
class LoadingScreen(QDialog):
    """Loading screen with icon and progress bar"""
    def __init__(self):
        super().__init__()
        self.setWindowTitle("OpenRailsLink")
        self.setModal(True)
        ...
        # Add stretch to push icon to center
        layout.addStretch()
        
        # Icon at center - larger to fill window
        icon_label = QLabel()
        icon_path = resource_path("icon.png")
        if os.path.exists(icon_path):
            pixmap = QPixmap(icon_path)
            icon_label.setPixmap(pixmap.scaled(280, 280, Qt.KeepAspectRatio, Qt.SmoothTransformation))
        else:
            icon_label.setText("OpenRailsLink")
            icon_label.setStyleSheet("font-size: 24px; font-weight: bold; color: #fff;")
        icon_label.setAlignment(Qt.AlignCenter)
        layout.addWidget(icon_label)
        
        # Status label - smaller text below icon
        self.status_label = QLabel("Initializing...")
        self.status_label.setAlignment(Qt.AlignCenter)
        self.status_label.setStyleSheet("font-size: 10px; color: #aaa; margin-top: 10px;")
        layout.addWidget(self.status_label)
        
        # Add stretch before progress bar to push it to bottom
        layout.addStretch()
...
class MainAppWindow(QMainWindow):
    ...
    def __init__(self, profile_path=None):
        ...
        # --- FIXED INITIALIZATION ORDER: Load config BEFORE UI ---
        self.load_app_config()  # ← Load config FIRST
        
        # Update loading screen if it exists
        if hasattr(QApplication.instance(), 'activeModalWidget'):
            modal = QApplication.instance().activeModalWidget()
            if modal and hasattr(modal, 'update_progress'):
                modal.update_progress(40, "Building user interface...")
        
        self.init_ui()          # ← Then build UI (which may read config)
        
        if hasattr(QApplication.instance(), 'activeModalWidget'):
            modal = QApplication.instance().activeModalWidget()
            if modal and hasattr(modal, 'update_progress'):
                modal.update_progress(60, "Connecting signals...")
        ...
        if hasattr(QApplication.instance(), 'activeModalWidget'):
            modal = QApplication.instance().activeModalWidget()
            if modal and hasattr(modal, 'update_progress'):
                modal.update_progress(70, "Detecting devices...")
        ...
        if hasattr(QApplication.instance(), 'activeModalWidget'):
            modal = QApplication.instance().activeModalWidget()
            if modal and hasattr(modal, 'update_progress'):
                modal.update_progress(75, "Starting web interface...")
        ...
        if self.saitek_manager.is_connected(): 
            self.log_message("Found Saitek Switch Panel.", "APP")
        self.web_interface.start()

        # Load profile (command-line arg takes priority, then default from config)
        self.log_message(f"🎯 Profile loading stage - profile_path parameter: {profile_path}", "DEBUG")

        if hasattr(QApplication.instance(), 'activeModalWidget'):
            modal = QApplication.instance().activeModalWidget()
            if modal and hasattr(modal, 'update_progress'):
                modal.update_progress(80, "Loading profile..." if profile_path else "Ready!")

        if profile_path:
            self.log_message(f"📥 Loading profile from command-line: {profile_path}", "APP")
            self.log_message(f"   File exists: {os.path.exists(profile_path)}", "DEBUG")
            self.load_profile(profile_path)
        else:
            # Load default profile from config
            default_profile = self.config.get("settings", {}).get("default_profile_path", "")
            self.log_message(f"🔍 Checking for default profile in config: '{default_profile}'", "APP")
            
            if default_profile:
                if os.path.exists(default_profile):
                    self.log_message(f"✓ Loading default profile: {default_profile}", "APP")
                    self.load_profile(default_profile)
                else:
                    self.log_message(f"⚠️ Default profile not found: {default_profile}", "APP")
                    self.log_message(f"   (File does not exist - profile may have been moved/deleted)", "APP")
            else:
                self.log_message("No default profile configured", "APP")
    ...
    def load_profile(self, path=None):
        self.log_message(f"🔍 load_profile() called with path: {path}", "DEBUG")
        if not path:
        ...
        try:
            self.log_message(f"📂 Attempting to parse XML file: {path}", "DEBUG")
            self.log_message(f"   File exists: {os.path.exists(path)}", "DEBUG")
            tree = etree.parse(path)
            self.log_message(f"✓ XML parsed successfully", "DEBUG")
            self.bindings.clear(); self.slider_last_values.clear()
    ...
    def load_app_config(self):
        # ALWAYS load config from the executable's directory, NOT from bundled resources
        ...
        try:
            with open(config_path, 'r') as f: 
                ...
                # Can't log yet - debug_log doesn't exist until init_ui()
                print(f"[CONFIG] Loaded from: {config_path}")
                print(f"[CONFIG] Default profile: '{self.config.get('settings', {}).get('default_profile_path', '')}'")
        except Exception as e:
            print(f"[CONFIG] Could not load from {config_path}: {e}")
            self.config = {"settings": {"default_profile_path": "","launcher_profiles": []}}
            self.save_app_config()
            
    def save_app_config(self):
        # Save config to the executable's directory
        if getattr(sys, 'frozen', False):
            path = os.path.join(os.path.dirname(sys.executable), "config.json")
        else:
            path = os.path.join(os.path.abspath("."), "config.json")
        
        print(f"[CONFIG] Saving to: {path}")
...
if __name__ == "__main__":
    ...
    # DEBUG: Print arguments to console
    print(f"[STARTUP] Command line arguments parsed:")
    ...
    loading.show()
    loading.update_progress(10, "Initializing application...")
    QApplication.processEvents()
    
    # Create main window with progress updates
    loading.update_progress(30, "Loading configuration...")
    window = MainAppWindow(profile_path=args.profile)
