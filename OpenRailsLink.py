...
from PyQt5.QtCore import Qt, QDateTime, QFileInfo, QSize, pyqtSignal, QMetaObject
...
class MainAppWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        trackir_warning = QLabel(
            "<b>⚠️ Important:</b> Before starting TrackIR integration, <b>move the camera in-game first</b> "
    ...
    def _get_base_cmd(self, script_name):
        return [resource_path(script_name.replace('.py', '.exe'))] if getattr(sys, 'frozen', False) else [sys.executable, resource_path(script_name)]

    def start_trackir_writer(self):
        if self.trackir_writer_process or not self.trackir_addresses.get('cab'): return
        ...
        with open(config_file, 'w') as f: json.dump(camera_configs, f)
        cmd = self._get_base_cmd("trackir_integration.py")
        cmd.extend(["--config-file", config_file]); [cmd.extend([f"--{k}-address", v[0]]) for k, v in self.trackir_addresses.items() if v]; cmd.extend(["--active-camera", self.trackir_active_camera])
        self.trackir_writer_process = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=False, creationflags=subprocess.CREATE_NO_WINDOW if sys.platform == "win32" else 0)
        threading.Thread(target=self._read_writer_output, daemon=True).start(); self.update_writer_button_states()

    def stop_trackir_writer(self):
        if not self.trackir_writer_process: return
        try:
            shutdown_flag = os.path.join(tempfile.gettempdir(), f"trackir_writer_shutdown_{self.trackir_writer_process.pid}.flag")
            with open(shutdown_flag, 'w') as f: f.write('1')
            try: self.trackir_writer_process.wait(timeout=5)
            except: self.trackir_writer_process.kill(); self.trackir_writer_process.wait()
        except: pass
        self.trackir_writer_process = None; self.update_writer_button_states()
    ...
    def start_camera_scan(self, camera_type):
        if self.trackir_scanner_processes[camera_type]: return
        ...
        self.trackir_scanner_processes[camera_type] = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=False, creationflags=subprocess.CREATE_NO_WINDOW if sys.platform == "win32" else 0)
        threading.Thread(target=self._read_scanner_output, args=(camera_type,), daemon=True).start()
        if camera_type == 'cab': self.trackir_restart_cab_scan_btn.setEnabled(True)
        self.update_writer_button_states()

    def restart_camera_scan(self, camera_type):
        self.stop_camera_scan(camera_type); time.sleep(0.5); self.start_camera_scan(camera_type)

    def stop_camera_scan(self, camera_type):
        if not self.trackir_scanner_processes[camera_type]: return
        try:
            flag = os.path.join(tempfile.gettempdir(), f"trackir_scanner_{camera_type}_shutdown_{self.trackir_scanner_processes[camera_type].pid}.flag")
            with open(flag, 'w') as f: f.write('1')
            try: self.trackir_scanner_processes[camera_type].wait(timeout=5)
            except: self.trackir_scanner_processes[camera_type].kill(); self.trackir_scanner_processes[camera_type].wait()
        except: pass
        self.trackir_scanner_processes[camera_type] = None
        if camera_type == 'cab': self.trackir_restart_cab_scan_btn.setEnabled(False)
        self.update_writer_button_states()
    ...
    def closeEvent(self, event):
        ...
        self.stop_trackir_writer(); [self.stop_camera_scan(c) for c in ['cab', 'external', 'interior']]; self.save_app_config(); self.joystick_manager.shutdown(); self.saitek_manager.shutdown(); self.web_interface.stop(); event.accept()
